// api/treasury/cashes.js
const express = require("express");
const { supabaseAdmin } = require("../../supabaseAdmin");
const authMiddleware = require("../middleware/auth");

const router = express.Router();

/* GET ALL CASHES */
router.get("/", authMiddleware, async (req, res) => {
    try {
        const { with_tafsili } = req.query;
        const member_id = req.user.id;

        let selectQuery = with_tafsili === 'true'
            ? "*, accounting_tafsili(id, code, title)"
            : "*";

        const { data, error } = await supabaseAdmin
            .from("treasury_cashes")
            .select(selectQuery)
            .eq("member_id", member_id) // ✅ فیلتر تنانت
            .order("created_at", { ascending: false });

        if (error) throw error;

        return res.json({ success: true, data });
    } catch (e) {
        return res.status(500).json({ success: false, error: e.message });
    }
});

/* GET ONE CASH */
router.get("/:id", authMiddleware, async (req, res) => {
    try {
        const { data, error } = await supabaseAdmin
            .from("treasury_cashes")
            .select("*, accounting_tafsili(id, code, title)")
            .eq("id", req.params.id)
            .eq("member_id", req.user.id)
            .single();

        if (error || !data) {
            return res.status(404).json({
                success: false,
                error: "صندوق یافت نشد یا دسترسی ندارید"
            });
        }

        return res.json({ success: true, data });
    } catch (e) {
        return res.status(500).json({ success: false, error: e.message });
    }
});

/* CREATE CASH */
router.post("/", authMiddleware, async (req, res) => {
    try {
        const member_id = req.user.id;

        const payload = {
            ...req.body,
            member_id,
            tafsili_id: null
        };

        delete payload.id;
        delete payload.created_at;

        if (!payload.title) {
            return res.status(400).json({
                success: false,
                error: "عنوان صندوق الزامی است"
            });
        }

        // 1. ساخت صندوق
        const { data: createdCash, error: cashError } = await supabaseAdmin
            .from("treasury_cashes")
            .insert([payload])
            .select()
            .single();

        if (cashError) throw cashError;

        // 2. ساخت تفصیلی
        try {
            const nextCode = await generateNextTafsiliCode(member_id, 'cash');

            const tafsiliData = {
                code: nextCode,
                title: payload.title,
                tafsili_type: 'cash',
                ref_id: createdCash.id,
                member_id: member_id,
                is_active: true
            };

            const { data: createdTafsili, error: tafsiliError } = await supabaseAdmin
                .from("accounting_tafsili")
                .insert([tafsiliData])
                .select()
                .single();

            if (!tafsiliError) {
                await supabaseAdmin
                    .from("treasury_cashes")
                    .update({ tafsili_id: createdTafsili.id })
                    .eq("id", createdCash.id);

                createdCash.tafsili_id = createdTafsili.id;
            }
        } catch (e) {
            console.error("⚠️ Tafsili creation failed:", e);
        }

        return res.json({
            success: true,
            data: createdCash,
            message: "صندوق با موفقیت ایجاد شد"
        });
    } catch (e) {
        return res.status(500).json({ success: false, error: e.message });
    }
});

/* UPDATE CASH */
router.put("/:id", authMiddleware, async (req, res) => {
    try {
        const cash_id = Number(req.params.id);
        const member_id = req.user.id;

        const { data: existing } = await supabaseAdmin
            .from("treasury_cashes")
            .select("id, tafsili_id")
            .eq("id", cash_id)
            .eq("member_id", member_id)
            .single();

        if (!existing) {
            return res.status(404).json({
                success: false,
                error: "صندوق یافت نشد یا دسترسی ندارید"
            });
        }

        const payload = { ...req.body };
        delete payload.id;
        delete payload.member_id;
        delete payload.created_at;
        delete payload.tafsili_id;

        const { data, error } = await supabaseAdmin
            .from("treasury_cashes")
            .update(payload)
            .eq("id", cash_id)
            .eq("member_id", member_id)
            .select()
            .single();

        if (error) throw error;

        // آپدیت تفصیلی
        if (payload.title && existing.tafsili_id) {
            await supabaseAdmin
                .from("accounting_tafsili")
                .update({ title: payload.title })
                .eq("id", existing.tafsili_id);
        }

        return res.json({
            success: true,
            data,
            message: "صندوق با موفقیت ویرایش شد"
        });
    } catch (e) {
        return res.status(500).json({ success: false, error: e.message });
    }
});

/* DELETE CASH */
router.delete("/:id", authMiddleware, async (req, res) => {
    try {
        const cash_id = Number(req.params.id);
        const member_id = req.user.id;

        const { data: cash } = await supabaseAdmin
            .from("treasury_cashes")
            .select("id, tafsili_id")
            .eq("id", cash_id)
            .eq("member_id", member_id)
            .single();

        if (!cash) {
            return res.status(404).json({
                success: false,
                error: "صندوق یافت نشد یا دسترسی ندارید"
            });
        }

        const { error } = await supabaseAdmin
            .from("treasury_cashes")
            .delete()
            .eq("id", cash_id)
            .eq("member_id", member_id);

        if (error) {
            if (error.code === '23503') {
                return res.status(409).json({
                    success: false,
                    error: "امکان حذف وجود ندارد (صندوق در تراکنش‌ها استفاده شده)"
                });
            }
            throw error;
        }

        // حذف تفصیلی
        if (cash.tafsili_id) {
            await supabaseAdmin
                .from("accounting_tafsili")
                .delete()
                .eq("id", cash.tafsili_id);
        }

        return res.json({
            success: true,
            message: "صندوق با موفقیت حذف شد"
        });
    } catch (e) {
        return res.status(500).json({ success: false, error: e.message });
    }
});

async function generateNextTafsiliCode(memberId, type) {
    try {
        const { data: lastRecord } = await supabaseAdmin
            .from("accounting_tafsili")
            .select("code")
            .eq("member_id", memberId)
            .eq("tafsili_type", type)
            .lt('code', '999999')
            .order("created_at", { ascending: false })
            .limit(1)
            .maybeSingle();

        let nextNum = 1;
        if (lastRecord && lastRecord.code && !isNaN(Number(lastRecord.code))) {
            nextNum = Number(lastRecord.code) + 1;
        }

        return String(nextNum).padStart(4, "0");
    } catch (e) {
        return "0001";
    }
}

module.exports = router;